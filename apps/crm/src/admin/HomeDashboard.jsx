...
/* ── Optin Rate box (Start Registration / Page Visits × 100) ── */
function OptinRateBox({ counts }) {
  const visits = counts.page_visited || 0;
  const regs = counts.cta_clicked || 0;
  const pct = visits > 0 ? Math.round((regs / visits) * 1000) / 10 : 0;

  const color = pct >= 50 ? '#059669' : pct >= 25 ? '#D97706' : '#DC2626';
  const ringBg = pct >= 50 ? 'rgba(5,150,105,0.12)' : pct >= 25 ? 'rgba(217,119,6,0.12)' : 'rgba(220,38,38,0.12)';
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{
          ...
          {regs.toLocaleString()} / {visits.toLocaleString()} (Start Registration ÷ Page Visits)
        </div>
      </div>
    </div>
  );
...
export default function HomeDashboard({ token, source = 'meta' }) {
  ...
  return (
    <div style={{ fontFamily: 'Outfit, sans-serif' }}>
      ...
      {/* ── Drop-off summary + Optin Rate (single row) ── */}
      {!loading && (
        <div className="dash-dropoff-grid" style={{
          display: 'grid',
          gridTemplateColumns: `repeat(${DROPOFF_BOXES.length + 1}, minmax(0, 1fr))`,
          gap: 12, marginBottom: 28,
        }}>
          {DROPOFF_BOXES.map(box => (
            <DropoffBox key={box.label} box={box} counts={counts} />
          ))}
          <OptinRateBox counts={counts} />
        </div>
      )}
