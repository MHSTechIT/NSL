/* One-shot: clear caller Hari's Assigned page and seed 20 fresh test leads.
   ----------------------------------------------------------------------
   1. Park every lead CURRENTLY ON Hari's Assigned page
      ...
      (admin starting a new batch, or a manual unpark, brings them back).
   2. Insert 20 test leads (whatsapp_number 9176753253) assigned to Hari
      on the active webinar so they show up on his Assigned page.

   Run from crm/backend:
     node scripts/seed-hari-9176-test-leads.js
*/

require('dotenv').config();
const pool = require('../db');

const TEST_NUMBER = '9176753253';
const COUNT       = 20;
...
(async () => {
  ...
  try {
    ...
    // 4. Insert 20 test leads, each with a unique full_name + email.
    ...
    for (let i = 1; i <= COUNT; i++) {
      ...
      const email    = `hari.test.9176.${stamp}.${i}@example.com`;
